...
class GuiEvaluator:
    ...
    def compare_img_with_gui(self, image_name, threshold=0.99) -> bool:
        current_image_path = os.path.join(self.example_folder, image_name)
        if self._gui_has_significant_change(current_image_path, threshold):
            self.gui_changed.append(current_image_path)
            return False
        return True
    ...
    def _gui_has_significant_change(self, current_image_path, threshold):
    ...
    def change_report(self):
        ...
        return dedent(
            ...
            If the autogenerated image seems incorrect the underlying code generating
